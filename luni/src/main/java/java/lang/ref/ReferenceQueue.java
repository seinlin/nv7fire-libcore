...
public class ReferenceQueue<T> {
    ...
    public synchronized Reference<? extends T> pollTail() {
        if (head == null) {
            return null;
        }

        Reference<? extends T> ret;
        Reference<? extends T> prev;

        ret = head;
        prev = ret;

        while (ret != ret.queueNext) {
            prev = ret;
            ret = ret.queueNext;
        }
        ...
        if (ret == head) {
            ...
            //remove from tail. new tail is prev
            if (prev != ret)
                prev.queueNext = prev;
        }
    ...
    public synchronized Reference<? extends T> removeTail()
